/* Red.cpp - main public implementation for RED - Regular Expression DFA

   See general description in Red.h

   A lot of this code is boilerplate generated by scripts/apigen in
   order to generate repetitive code without intorducing human error.

   A main reason why there are so many entrypoints is to avoid run-time
   decisions about things that are often known at compile time.  In
   certain applications, this level of optimization can matter.
*/
